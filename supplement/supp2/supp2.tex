...
\subsubsection{Assuming a normal distribtion of transformed abundances}
...
## T0.3 - Control >= 0  -0.9558     0.4536  -2.107 0.1032  
## T1 - Control >= 0    -1.0069     0.4536  -2.220 0.0859 .
## T3 - Control >= 0    -1.4121     0.4536  -3.113 0.0184 *
...
This indicates that only treatment 3 shows a statistically significant from control and is therefore the determined LOEC.
...
Here, I also added the assumed mean-variance relationships of the Poisson, quasi-Poisson and negative binomial models.
...
## Linear Hypotheses:
##                     Estimate Std. Error z value Pr(<z)  
## T0.1 - Control >= 0  -0.5092     0.3889  -1.309 0.3514  
## T0.3 - Control >= 0  -0.9970     0.4656  -2.142 0.0737 .
## T1 - Control >= 0    -0.8560     0.4409  -1.941 0.1153  
...
For the LR-Test we need to first fit a reduced model:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
## Linear Hypotheses:
##                     Estimate Std. Error z value  Pr(<z)    
## T0.1 - Control >= 0  -0.5092     0.3951  -1.289 0.31530    
## T0.3 - Control >= 0  -0.9970     0.3988  -2.500 0.02747 *  
## T1 - Control >= 0    -0.8560     0.3975  -2.153 0.06494 .  
## T3 - Control >= 0    -1.6032     0.4066  -3.943 < 0.001 ***
## T10 - Control >= 0   -1.4313     0.4601  -3.111 0.00444 ** 
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## (Adjusted p values reported -- single-step method)
\end{verbatim}
...
\hlstd{dfm}\hlopt{$}\hlstd{y_asin} \hlkwb{<-} \hlkwd{ifelse}\hlstd{(dfm}\hlopt{$}\hlstd{y} \hlopt{==} \hlnum{1}\hlstd{,}
                     \hlkwd{asin}\hlstd{(}\hlnum{1}\hlstd{)} \hlopt{-} \hlkwd{asin}\hlstd{(}\hlkwd{sqrt}\hlstd{(}\hlnum{1}\hlopt{/}\hlnum{10}\hlstd{)),}
                     \hlkwd{ifelse}\hlstd{(dfm}\hlopt{$}\hlstd{y} \hlopt{==} \hlnum{0}\hlstd{,}
                            \hlkwd{asin}\hlstd{(}\hlkwd{sqrt}\hlstd{(}\hlnum{1}\hlopt{/}\hlnum{10}\hlstd{)),}
...
## Residuals:
##      Min       1Q   Median       3Q      Max 
## -0.28326 -0.03547  0.00000  0.03547  0.18038 
## 
## Coefficients:
##               Estimate Std. Error t value Pr(>|t|)    
## (Intercept)  1.249e+00  5.463e-02  22.866 9.44e-15 ***
## conc32      -1.064e-01  7.725e-02  -1.378   0.1852    
## conc64      -1.144e-16  7.725e-02   0.000   1.0000    
## conc128     -3.547e-02  7.725e-02  -0.459   0.6516    
## conc256     -1.804e-01  7.725e-02  -2.335   0.0313 *  
## conc512     -6.459e-01  7.725e-02  -8.360 1.30e-07 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 0.1093 on 18 degrees of freedom
## Multiple R-squared:  0.8507,	Adjusted R-squared:  0.8092 
## F-statistic: 20.51 on 5 and 18 DF,  p-value: 7.323e-07
\end{verbatim}
\end{kframe}
\end{knitrout}
...
## <none>              0.21484 -101.181                      
## conc    5    1.2242 1.43903  -65.538  20.513 7.323e-07 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}
\end{kframe}
...
## Linear Hypotheses:
##                Estimate Std. Error t value Pr(<t)    
## 32 - 0 >= 0  -1.064e-01  7.725e-02  -1.378 0.2695    
## 64 - 0 >= 0  -1.144e-16  7.725e-02   0.000 0.8333    
## 128 - 0 >= 0 -3.547e-02  7.725e-02  -0.459 0.6652    
## 256 - 0 >= 0 -1.804e-01  7.725e-02  -2.335 0.0573 .  
## 512 - 0 >= 0 -6.459e-01  7.725e-02  -8.360 <0.001 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## (Adjusted p values reported -- single-step method)
\end{verbatim}
...
## Linear Hypotheses:
##              Estimate Std. Error z value Pr(<z)    
## 32 - 0 >= 0   -1.2098     0.8499  -1.423 0.2033    
## 64 - 0 >= 0    0.7191     1.2458   0.577 0.9229    
## 128 - 0 >= 0  -0.7472     0.8967  -0.833 0.4320    
## 256 - 0 >= 0  -1.7077     0.8183  -2.087 0.0589 .  
