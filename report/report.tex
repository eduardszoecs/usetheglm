...
We first use a data set from a mesocosm study to illustrate differences between statistical methods.
Then we further elaborate differences using simulations of two data types commonly encountered in ecotoxicology: counts and proportions.
Methods were compared in terms of Type 1 error (maintain a significance level of 0.05 when there is no effect) and power (detect an effect when it is present).
...
We simulated count data that mimics of the case study with five treatments (T1 - T5) and one control group (C).
...
\subsubsection{Generalized Linear Models}
Data of type \emph{x out of N} can be modelled by a binomial distribution with parameters N and $\pi$ ($GLM_b)$:
...
Following general recommendations \citep{bolker_generalized_2009,faraway_extending_2006}, we used F-tests ($LM$ and $GLM_{qp}$) and Likelihood-Ratio (LR) tests ($GLM_p$, $GLM_{nb}$ and $GLM_b$) to test the first hypothesis.
However, it is well known that LR test are unreliable with small sample sizes \citep{wilks_large-sample_1938}.
Therefore, we additionally explored parametric bootstrap \citep{faraway_extending_2006} to assess the significance of the LR for the negative binomial models (denoted as $GLM_PB$, using 500 bootstrap samples).
...
Beside these parametric methods we also applied two non-parametric methods: The Kruskal-Wallis test  ($KW$) to test for a general treatment effect and a pairwise Wilcoxon test ($WT$) to determine the LOEC.
...
For detecting a treatment effect $GLM_{nb}$, $GLM_{pb}$ and $GLM_{qp}$ exhibited higher power then $LM$ and $KW$, the latter having least power.
...
\todo{$GLM_{qp}$ showed up to 17\% greater power compared to $LM$. -> discussion}
\todo{Abbreviations to methods}

The inferences on LOEC generally showed less power.
For $LM$ this reduction was up to 35\% (n = 9, $\mu_C$ = 64, Figures \ref{fig:p_glob_c} and \ref{fig:p_loec_c}).
Power to detect the LOEC was highest for $GLM_{nb}$ and $GLM_{pb}$. $LM$ and WT showed less power, and WT had no power to detect the correct LOEC at low sample sizes.
...
\subsubsection{Binomial data}
Binomial GLM showed the greatest power for testing the treatment effect. 
This was especially apparent at low sample sizes (n = 3), with up to 24\% higher power compared to LM.
KW had the lowest power and slightly conservative.
...
Contrary to the global test, $LM$ showed the higher power than $GLM_{b}$ at small sample sizes.
...
WT had no power at all for n = 3 and showed less power in the other simulation runs.
$LM$ maintained a Type 1 error level of 0.05 in all simulations. 
$GLM_b$ was conservative at small effect sizes ($p_E > 0.8$) and WT was generally conservative showing lowered Type 1 error rates.
...
\subsection{Case study}
%% ---- Case study
This case study demonstrates that the choice of the statistical model and procedure can have substantial impact on ecotoxicological inferences.
This is further highlighted by the fact that \citet{brock_minimum_2015} reported a LOEC of \mbox{0.3 mg/L} for this data assuming normality of transformed data.
The reasons can be manyfold: \citep{brock_minimum_2015} used a $log(2~y + 1)$ transformation, whereas we used a $log(A~y + 1)$ transformation, where A = 2 / 11 = 0.182 \citep{van_den_brink_impact_2000}.
...
However, in factorial designs the mean-variance relationship can be easily checked by plotting mean vs. variance of the treatment groups.
In the introduction we pointed out that there is little advice how to choose between the plenty of possible transformations.
How do GLMs simplify this problem?
The distribution modelled should be chosen to give a statistically sound model reflecting properties of the data (e.g. bonds, integer or continuous data etc.).
Moreover, the model selection process can be guided by the data and diagnostic plots and therefore, it is much more sound than choosing between transformations.
\todo{zu holperig}
...
For example, extremely low samples sizes (n \textless 5) are common in many cases \citep{sanderson_pesticide_2002,szocs_analysing_2015}.
...
Though more powerful non-parametric tests may be available \citep{konietschke_rank-based_2012}, these are focused on testing but not on estimation of effects.
...
Therefore, we do advise to use GLMs instead of non-parametric tests for non-normal data.
...
At small samples sizes and / or low abundances it is difficult to determine the optimal mean-variance relationship.
...
\section{Conclusions}
\label{sec:concl}
Statistical hypothesis tests are commonly used in ecotoxicology to make inferences on pollutant effects.
...
We cannot recommend the current practice to either transform the data or use non-parametric approaches if data is not normally distributed.
