...
Here, we compare the statistical power of analyses using transformed data or non-parametric methods to analyses using appropriate models, namely Generalised Linear Models (GLM).
...
In addition, we outlined differences and advantages of GLMs on a real world data set.

We found that GLMs provide a gain a gain in statistical power compared to analysis of transformed data or using non-parametric methods.
GLMs should become a standard method in ecotoxicology and we recommend that non-normal data should be analysed by GLMs and not by transformations or non-parametric methods.
...
Although their use has been heavily criticized in the past \citep{landis_well_2011},  they are still regularly used in ecotoxicology \citep{jager_bad_2012}.
...
%% --------------------------------
\subsection{Case study}
The data set set show considerable overdispersion ($\Theta = 22.41$, eqn. \ref{eqn:quasi}).
...
\begin{figure}
  ...
  For n = 3 and $\mu_C$ = \{2, 4\} less then 80\% of $GLM_{nb}$ and $GLM_{pb}$ models did converge.
...
Contrary to the global test, $LM$ showed the higher power than $GLM_{bin}$ at small sample sizes.
...
Knowing what type of data is modelled (see Methods section), the model selection process can be completely guided by the data and diagnostic plots. Therefore, choosing a appropriate model is much more sound and straightforward than choosing between possible transformations.


\subsection{Simulations}
%% --- general low power
Our simulations show that GLMs have greater power than data transformations.
However, the simulations also suggest that power in common mesocosm experiments is low.
...
Statistical power to detect the correct LOEC was even further reduced, with power less than 30\%.
This suggests that NOECs reported from mesocosm experiments should be interpreted with caution and underpins the criticism of NOEC.
...
Therefore, we advise to use GLMs instead of non-parametric tests for non-normal data.
...
It is well known that the LR statistic is unreliable for small sample sizes \citep{bolker_generalized_2009,wilks_large-sample_1938} and we found that parametric bootstrap ($GLM_{pb}$) provides a valuable alternative in such situations (Figure \ref{fig:p_glob_c}).
