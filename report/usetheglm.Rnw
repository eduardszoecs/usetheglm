...
In environmental risk assessments statistical tests play an important role to evaluate the effects of pesticides. Despite criticism (e.g. \citet{landis_well_2011}) summary statistics like the Lowest Observed Effect Concentration (LOEC) or No Observed Effect Concentration (NOEC) are still regularly used to report results of dose response experiments.
One issue with NOEC is the statistical power in the experiments, i. e. the ability to detect an effect.

Recently, \citet{brock_minimum_2014} proposed two methods to decrease the minimum detectable difference (MDD) in mesocosm experiment:  increasing the number of replicates (impractical for mesocosms) or by increasing the sampling efficiency. In this paper we present a third method to increase the statistical power, namely the use of Generalized Linear Models (GLM) \citep{nelder_generalized_1972}.
...
proportions of surviving animals (bonded between 
However, there is also a third possibility:
Using a distribution fitting to the type of data instead of the normal distribution, namely \emph{Generalized Linear Models} (GLM) \citep{nelder_generalized_1972}. 0 and 1) or biomass (positive data).
...
However, there is also a third possibility:
Using a distribution fitting to the type of data instead of the normal distribution.
GLMs can handle various types of data distributions, e.g. poisson or negative binomial (counts), binomial (proportions) or gamma (mass); the normal distribution being a special case of GLMs.
...
We used simulated data that mimicked data encountered in mesocosm experiments.
...
Additionally we also analysed this data using a non-parametric approach.
