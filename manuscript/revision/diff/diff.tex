...
%DIF ADD revision/report/report.tex     Mon Mar 16 16:55:33 2015
...
% latexdiff submission/report/report.bbl revision/report/report.bbl > revision/diff/diff.bbl %DIF > 
 %DIF > 
% % then document %DIF > 
% latexdiff --disable-citation-markup --math-markup=0 submission/report/report.tex revision/report/report.tex > revision/diff/diff.tex %DIF > 
 %DIF > 
 %DIF > 
%DIF -------
%% ----------------------------------------------------------------------------
...
%DIFDELCMD <   y^T_i & = log(Ay_i + 1) \label{eqn:trans} \\
%DIFDELCMD <   A & = 2~/~min(y)~~~\text{, for}~ y > 0 \nonumber
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
...
, where $y_i$ is the measured \DIFdelbegin \DIFdel{abundance }\DIFdelend and $y_i^T$ the transformed abundance. 
\DIFaddbegin \DIFadd{The factor $A$ was chosen in such way that $Ay$ equals 2 for the lowest non-zero abundance value (y).
}\DIFaddend 
...
All \DIFdelbegin \DIFdel{computations }\DIFdelend \DIFaddbegin \DIFadd{simulations }\DIFaddend were done in R (Version 3.1.2) \citep{r_core_team_r:_2014} on a \DIFdelbegin \DIFdel{Linux machine}\DIFdelend \DIFaddbegin \DIFadd{Amazon EC2 Linux instance (64bit, 15GB RAM, 8 cores, 2.8 GHz)}\DIFaddend .
...
\subsection{Simulations}
\subsubsection{Count data}
For \DIFaddbegin \DIFadd{detecting a general treatment effect $GLM_{nb}$ and $GLM_{p}$ showed inflated type 1 error rates, whereas $KW$ was conservative at low sample sizes (Figure \ref{fig:p_glob_c}).
Using parametric bootstrap this could be fixed for the negative binomial model.
For detecting a treatment effect $GLM_{npb}$ and $GLM_{qp}$ exhibited higher power than $LM$ and $KW$, the latter having least power.
...
\begin{figure*}
  ...
  \caption{Count data simulations \DIFdelbeginFL \DIFdelFL{: Power }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{top}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1000 simulated datasets}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: }\DIFaddendFL Type 1 error (\DIFaddbeginFL \DIFaddFL{top) and Power (}\DIFaddendFL bottom) for the test of a treatment effect.
...
\DIFaddbegin \DIFadd{For LOEC determination $GLM_{nb}$ and $GLM_{p}$ showed an increased Type 1 error and all other methods being slightly conservative (Figure \ref{fig:p_loec_c}).
...
The power }\DIFdelend \DIFaddbegin \DIFadd{showed a mean reduction of 20.7\% and $GLM_qp$ 24.3 \%.
Power }\DIFaddend to detect the LOEC was highest for \DIFdelbegin \DIFdel{$GLM_{nb}$ and $GLM_{npb}$}\DIFdelend \DIFaddbegin \DIFadd{$GLM_{qp}$}\DIFaddend . $LM$ and \DIFdelbegin \DIFdel{WT }\DIFdelend \DIFaddbegin \DIFadd{$WT$ }\DIFaddend showed less power, with \DIFdelbegin \DIFdel{WT }\DIFdelend \DIFaddbegin \DIFadd{$WT$ }\DIFaddend having no power to detect the LOEC at low sample sizes.
\DIFdelbegin \DIFdel{At low sample sizes $GLM_{nb}$ showed an increased Type 1 error and WT was slightly conservative (Figure \ref{fig:p_loec_c}).
}\DIFdelend 
...
\begin{figure*}
  ...
  \caption{Count data simulations \DIFdelbeginFL \DIFdelFL{: Power }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{top}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1000 simulated datasets}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{: }\DIFaddendFL Type 1 error (\DIFaddbeginFL \DIFaddFL{top) and Power (}\DIFaddendFL bottom) for determination of LOEC.
