...
%DIF ADD revision/report/report.tex     Mon Mar 16 22:20:22 2015
...
\begin{align}
  y^T \DIFdelbegin \DIFdel{_i }\DIFdelend & = log(Ay \DIFdelbegin \DIFdel{_i }\DIFdelend + 1) \label{eqn:trans}
...
, where \DIFdelbegin \DIFdel{$y_i$ }\DIFdelend \DIFaddbegin \DIFadd{$y$ }\DIFaddend is the measured \DIFdelbegin \DIFdel{abundance and $y_i^T$ }\DIFdelend \DIFaddbegin \DIFadd{and $y^T$ }\DIFaddend the transformed abundance. 
...
\DIFdelend = \sigma^2 \DIFaddbegin \label{eqn:normal} \\
    \DIFadd{\mu_i }&\DIFadd{= \beta X_i  }\DIFaddend \nonumber
\end{align}
...
$X_i$ is a set of explanatory variables coding the treatments and }\DIFaddend $\beta$ \DIFdelbegin \DIFdel{) are directly interpretable as changes from the control group }\DIFdelend \DIFaddbegin \DIFadd{are the parameters to be estimated from the data }\DIFaddend (eqn. \ref{eqn:normal}).
...
\begin{align}
  y_i &\sim P(\lambda_i) \nonumber \\
  \DIFaddbegin \DIFadd{E(y_i) }&\DIFadd{= var(y_i) = \lambda_i }\nonumber \\
  \DIFaddend log(\lambda_i) &= \mu_i \label{eqn:pois} \\
  \mu_i &= \DIFdelbegin \DIFdel{\alpha + \beta x}\DIFdelend \DIFaddbegin \DIFadd{\beta X}\DIFaddend _i \nonumber \\ 
\DIFdelbegin \DIFdel{var(y_i) }%DIFDELCMD < &%%%
...
However, these methods are not further investigated in this paper (see }\citealt{warton_arcsine_2011} \DIFadd{for a comparison)}\DIFaddend .
...
\begin{figure*}
  ...
  \caption{Count data simulations: \DIFdelbeginFL \DIFdelFL{Power }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Type 1 error }\DIFaddendFL (top) and \DIFdelbeginFL \DIFdelFL{Type 1 error }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Power }\DIFaddendFL (bottom) for the test of a treatment effect.
...
\begin{figure*}
  ...
  \caption{Count data simulations: \DIFdelbeginFL \DIFdelFL{Power }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Type 1 error }\DIFaddendFL (top) and \DIFdelbeginFL \DIFdelFL{Type 1 error }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Power }\DIFaddendFL (bottom) for determination of LOEC.
...
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 129mm]{p_loec_p.pdf}
  \DIFaddendFL \caption{
  Binomial data simulations: 
