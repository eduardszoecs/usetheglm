...
%DIF ADD revision/report/report.tex     Mon Mar 23 13:09:48 2015
...
% latexdiff submission/report/report.bbl revision/report/report.bbl > revision/diff/diff.bbl %DIF > 
 %DIF > 
% % then document %DIF > 
% latexdiff --disable-citation-markup --math-markup=3 --append-context2cmd=subtitle submission/report/report.tex revision/report/report.tex > revision/diff/diff.tex %DIF > 
 %DIF > 
 %DIF > 
 %DIF > 
%DIF -------
...
\subtitle{\DIFdelbegin \DIFdel{How the use of proper statistical models can increase statistical power in ecotoxicological experiments}\DIFdelend \DIFaddbegin \DIFadd{A comparison of statistical approaches for analysis of count and proportion data in ecotoxicology}\DIFaddend .}
...
%% ----------------------------------------------------------------------------
\begin{abstract}
\DIFdelbegin \DIFdel{Ecotoxicologists are often confronted with non-normally distributeddata}\DIFdelend \DIFaddbegin \DIFadd{Counts and proportions are data types often encountered by ecotoxicologists, which are rarely normally distributed}\DIFaddend .
...
\DIFaddbegin \DIFadd{Generalised Linear Models (GLM) allow directly model distributions fitting such data.
}\DIFaddend Here, we compare the \DIFdelbegin \DIFdel{statistical power of analyses using transformed dataor }\DIFdelend \DIFaddbegin \DIFadd{performance of parametric methods assuming (1) normality of transformed data, (2) appropriate distributions (Poisson, negativ binomial, binomial) and (3) }\DIFaddend non-parametric \DIFdelbegin \DIFdel{methods to analyses using appropriate distributional assumptions, }%DIFDELCMD < \allowbreak %%%
\DIFdel{namely Generalised Linear Models (GLM) }\DIFdelend \DIFaddbegin \DIFadd{methods}\DIFaddend .

We simulated \DIFdelbegin \DIFdel{data mimicking }\DIFdelend \DIFaddbegin \DIFadd{typical data mimicking low replicated }\DIFaddend ecotoxicological experiments of two common data types (counts and proportions \DIFaddbegin \DIFadd{from counts}\DIFaddend ). 
...
Nevertheless, they are \DIFdelbegin \DIFdel{usually }\DIFdelend \DIFaddbegin \DIFadd{often }\DIFaddend analysed using methods assuming a normal distribution and variance homogeneity \citep{wang_making_2011}. 
...
This model assumes a normal \DIFdelbegin \DIFdel{distributed response with constant variance ($\sigma^2$).
Note, that we parameterised the model as contrast ($\beta x_i$) to the control group ($\alpha$) so that parameters (}\DIFdelend \DIFaddbegin \DIFadd{distribution of the transformed abundances.
...
The response variable is linked to the predictors via a log-function }\DIFdelend \DIFaddbegin \DIFadd{This model assumes poisson distributed abundances with mean $\mu_i \ge 0$.
...
Therefore, $\beta$ also describes the differences between treatments on a log scale }\DIFaddend (eqn. \ref{eqn:pois}).
...
\DIFdelbegin \DIFdel{In both cases the parametrisation and link function is equal }\DIFdelend \DIFaddbegin \DIFadd{This models assumes that abundances are negative binomially distributed, with a mean of $\mu_i \ge 0$ and a variance $\mu_i + \mu_i^2 / \kappa$.
Similar }\DIFaddend to the Poisson \DIFdelbegin \DIFdel{GLM (eqn.
\ref{eqn:pois}).
}\DIFdelend \DIFaddbegin \DIFadd{model we use a log link between mean and treatments.
...
\DIFaddbegin \DIFadd{This model assumes that the number of occurences are binomially distributed}\DIFaddend , where N = number of \DIFdelbegin \DIFdel{exposed animals and $\pi$ }\DIFdelend \DIFaddbegin \DIFadd{trials (e.g. exposed animals) and $\pi_i$ }\DIFaddend is the probability of \DIFdelbegin \DIFdel{survival}\DIFdelend \DIFaddbegin \DIFadd{occurrences (fish survived), which together give the expected number of occurrences}\DIFaddend .
...
\DIFadd{Similarly to counts, binomial data may also show overdispersion. 
...
\subsection{Simulations}
\subsubsection{Count data}
For \DIFdelbegin \DIFdel{our simulation design (reduction in abundance by 50\%) a sample size per treatment of n = 9 was needed to achieve a power greater than 80\%.
}\DIFdelend \DIFaddbegin \DIFadd{detecting a general treatment effect, $GLM_{nb}$ and $GLM_{p}$ showed inflated type 1 error rates, whereas $KW$ was conservative at low sample sizes.
However, using parametric bootstrap for the negative binomial model ($GLM_{npb}$) resulted in appropriate type 1 error rates.
}\DIFaddend For detecting a treatment effect\DIFdelbegin \DIFdel{$GLM_{nb}$}\DIFdelend , $GLM_{npb}$ and $GLM_{qp}$ exhibited higher power than $LM$ and $KW$, the latter having least power \DIFdelbegin \DIFdel{.
Type 1 error rate was inflated for $GLM_{nb}$, but this could be fixed by using parametric bootstrap.
KW was conservative at low sample sizes }\DIFdelend (Figure \ref{fig:p_glob_c}).
\DIFaddbegin \DIFadd{For our simulation design (reduction in abundance by 50\%) a sample size per treatment of n = 9 was needed to achieve a power greater than 80\%.
}\DIFaddend At small sample sizes (n = {3, 6}) and low abundances ($\mu_C$ = {2, 4}) many of the negative binomial models ($GLM_{nb}$ and $GLM_{npb}$) did not converge to a solution (convergence rate \textless \DIFdelbegin \DIFdel{80}\DIFdelend \DIFaddbegin \DIFadd{85}\DIFaddend \% of the simulations, Supplement 1). 
...
\DIFaddbegin \DIFadd{For LOEC determination $GLM_{nb}$ and $GLM_{p}$ showed an increased Type 1 error and all other methods were slightly conservative.
...
In addition, $GLM_{bin}$ exhibited }\DIFaddend the greatest power for testing the treatment effect. 
...
\begin{figure*}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 129mm]{p_glob_p.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 126mm]{p_glob_p.pdf}
  \DIFaddendFL \caption{
...
\DIFdelendFL \DIFaddbeginFL \begin{figure*}
  \DIFaddendFL \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 84mm]{p_loec_p.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 126mm]{p_loec_p.pdf}
  \DIFaddendFL \caption{
  Binomial data simulations: 
...
The reasons are \DIFdelbegin \DIFdel{be }\DIFdelend manifold: \citep{brock_minimum_2015} used a $log(2~y + 1)$ transformation, whereas we used a $log(A~y + 1)$ transformation, where A = 2 / 11 = 0.182 \citep{van_den_brink_impact_2000}.
...
In contrast, we used a }\DIFdelend \DIFaddbegin \citep{williams_comparison_1972}\DIFadd{, whereas we used }\DIFaddend one-sided \DIFdelbegin \DIFdel{Dunnett test, which does not assume monotonicity and allows }\DIFdelend \DIFaddbegin \DIFadd{comparisons to the control (Dunnett contrasts).
In contrast to the Williams test, Dunnett contrasts do not assume a monotonic dose-response relationship and allow }\DIFaddend individual comparisons between treatment groups and the control\DIFdelbegin \DIFdel{, but has under monotonicity less power }\DIFdelend \DIFaddbegin \DIFadd{.
However, under monotonicity they have less power, which explains the differences }\DIFaddend \citep{jaki_statistical_2013}.
\DIFaddbegin \DIFadd{Both types of multiple comparisons are available as multiple contrast tests in a GLM framework.
...
For common samples sizes \DIFaddbegin \DIFadd{($n \le 4$ ) }\DIFaddend and a reduction in abundance of 50\% we found a low power to detect any treatment-related effect (\textless 50\% for methods with appropriate Type 1 error, Figure \ref{fig:p_glob_c}).
...
}\DIFdelend Statistical power to detect the correct LOEC was even lower (less than \DIFdelbegin \DIFdel{30\%).
This suggests that population level NOECs reported from mesocosm experiments }\DIFdelend \DIFaddbegin \DIFadd{25\%), which can be attributed to multiple testing.
The low power of all methods to detect significant treatment levels such as the LOEC or NOEC suggests that these endpoints from ecotoxicological studies }\DIFaddend should be interpreted with caution and underpins \DIFdelbegin \DIFdel{the criticism of NOEC }\DIFdelend \DIFaddbegin \DIFadd{their criticism }\DIFaddend \citep{laskowski_good_1995,landis_well_2011}.
...
However, }\citet{ter_braak_topics_2014} \DIFadd{argue to use data transformations with community data because of their simplicity and robustness.
...
%% --- binomial data
Binomial data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend often collected in lab trials, where increasing the sample size \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{may be relatively }\DIFaddend easy to accomplish. 
...
\DIFdelbegin \DIFdel{Nevertheless, for deriving LOECs the transformation performed better }\DIFdelend \DIFaddbegin \DIFadd{Though we did not simulate overdispersed binomial data, this should be checked and accounted for.
...
\DIFaddbegin \DIFadd{GLMs can be fitted with several statistical software packages and many textbooks are available to introduce ecotoxicologists to these models (e.g. }\citealt{zuur_beginners_2013} \DIFadd{or }\citealt{quinn_experimental_2009}\DIFadd{).
