...
%DIF ADD revision/report/report.tex     Fri Mar 20 15:56:46 2015
...
Recent studies concluded that data transformations should be avoided and GLMs be used as they have better statistical properties \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{ohara_not_2010, warton_arcsine_2011}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\citealt{ohara_not_2010}
}%DIFAUXCMD
(counts), \mbox{%DIFAUXCMD
\citealt{warton_arcsine_2011,warton_many_2005}
}%DIFAUXCMD
(proportions from counts))}\DIFaddend . 
...
Although their use has been heavily criticized in the past \citep{laskowski_good_1995}, they are \DIFdelbegin \DIFdel{still regularly used in ecotoxicology \mbox{%DIFAUXCMD
\citep{jager_bad_2012}
}%DIFAUXCMD
.
...
\DIFaddend \subsubsection{Linear model for transformed data}
To accommodate the assumptions for the standard linear model \DIFaddbegin \DIFadd{with such proportions}\DIFaddend , a special arcsine square root transformation (eqn. \ref{eqn:arcsine}) is suggested \DIFdelbegin \DIFdel{for such  data \mbox{%DIFAUXCMD
\citep{epa_methods_2002,newman_quantitative_2012}
}%DIFAUXCMD
:
}%DIFDELCMD < 
...
However, these are not further investigated in this paper (see \mbox{%DIFAUXCMD
\citet{warton_arcsine_2011}
}%DIFAUXCMD
for a comparison)}\DIFaddend .
...
To assess the LOEC we used Dunnett contrasts \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{dunnett_multiple_1955}
}%DIFAUXCMD
}\DIFaddend with one-sided Wald t tests (normal and quasi-Poisson models) and one-sided Wald Z tests (Poisson, negative binomial and binomial models).
...
\DIFaddbegin \DIFadd{We adjusted for multiple testing using the method of \mbox{%DIFAUXCMD
\citet{holm_simple_1979}
}%DIFAUXCMD
.
}\DIFaddend 
...
We simulated data from a commonly used design as \DIFdelbegin \DIFdel{in \mbox{%DIFAUXCMD
\citep{weber_short-term_1989}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{described in \mbox{%DIFAUXCMD
\citet{weber_short-term_1989}
}%DIFAUXCMD
}\DIFaddend , with 5 treated (T1 - T5) and a control group (C). 
...
\DIFaddbegin \DIFadd{Moreover, Supplement 2 provides worked examples of the data of \mbox{%DIFAUXCMD
\citet{brock_minimum_2015}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{weber_short-term_1989}
}%DIFAUXCMD
...
\DIFaddbegin \DIFadd{Therefore, ecotoxicologists should not base their inferences solely on statistical significance tests, but also on parameter estimates, their uncertainty and importance \mbox{%DIFAUXCMD
\citep{gelman_difference_2006}
}%DIFAUXCMD
.
Nevertheless, \mbox{%DIFAUXCMD
\citet{ohara_not_2010}
}%DIFAUXCMD
showed that $LM$ using a log transformation gave unreliable and biased parameter estimates, whereas GLMs performed well with little bias.
Bias occurs also when back-transforming means to the original scale, which explains the lower predicted means by $LM$ in Figure \ref{fig:example} \mbox{%DIFAUXCMD
\citep{rothery_cautionary_1988}
}%DIFAUXCMD
and should be corrected for \mbox{%DIFAUXCMD
\citep{newman_regression_1993}
}%DIFAUXCMD
.
}
...
\DIFdelbegin \DIFdel{Additionally, \mbox{%DIFAUXCMD
\citet{ohara_not_2010}
}%DIFAUXCMD
showed that using a log transformation gave unreliable and biased parameter estimates.
...
For community analyses \emph{GLM for multivariate data} \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{warton_distance-based_2012}
}%DIFAUXCMD
}\DIFaddend have been proposed as alternative to Principal Response Curves (PRC) and yielded to similar inferences, but better indication of responsive taxa \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{warton_distance-based_2012,szocs_analysing_2015}
}%DIFAUXCMD
. 
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{szocs_analysing_2015}
}%DIFAUXCMD
. 
However, \mbox{%DIFAUXCMD
\citet{ter_braak_topics_2014}
}%DIFAUXCMD
argue to use data transformations with community data because of their easy- and robustness.
...
However, it is well known that the LR statistic is not reliable at }\DIFaddend small sample sizes \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bolker_generalized_2009,wilks_large-sample_1938}
}%DIFAUXCMD
.
...
\DIFdelbegin \DIFdel{Moreover, the Likelihood-Ratio test gave an increased Type-I error for these models, where the non reliability of the LR statistic for small sample sizes has long been reported \mbox{%DIFAUXCMD
\citep{bolker_generalized_2009,wilks_large-sample_1938}
}%DIFAUXCMD
. 
...
At }\DIFdelend \DIFaddbegin \DIFadd{, top).
$GLM_{qp}$ assumes a simpler (linear) mean-variance relationship than the quadrativ relationship in $GLM_{npb}$. 
This might explain these differences between these two methods, because at }\DIFaddend small samples sizes, low abundances or few treatment groups it is difficult to determine the mean-variance relationship.
...
}\DIFdelend \DIFaddbegin \todo{Absatz nochmal drüberlesen}
\DIFaddend 
...
In such situations a GLMM may offer an appealing alternative \mbox{%DIFAUXCMD
\citep{warton_arcsine_2011}
}%DIFAUXCMD
.
...
This is because $\pi$ approaches its boundary and is also known as the }\emph{\DIFadd{Hauck-Donner effect}} \DIFadd{\mbox{%DIFAUXCMD
\citep{hauck_walds_1977}
}%DIFAUXCMD
. A LR-Test or parametric bootstrap may provide an alternative in such situations \mbox{%DIFAUXCMD
\citep{bolker_generalized_2009}
}%DIFAUXCMD
.
This can explain why $LM$ performed better for deriving LOECs }\DIFaddend at low sample sizes \DIFdelbegin \DIFdel{(n = 3)(Figure \ref{fig:p_loec_p}).
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend 

\DIFaddbegin \DIFadd{GLMs can be fitted with many statistical software packages and a lot of textbooks are available to introduce ecotoxicologists to these models (e.g. \mbox{%DIFAUXCMD
\citealt{zuur_beginners_2013}
}%DIFAUXCMD
or \mbox{%DIFAUXCMD
\citealt{quinn_experimental_2009}
}%DIFAUXCMD
).
