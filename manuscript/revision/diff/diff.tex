...
%DIF ADD revision/report/report.tex     Wed Mar 18 20:34:32 2015
...
}\DIFdelend \DIFaddbegin \DIFadd{Counts and proportions from counts are data types of encountered by ecotoxicologists.
...
}\DIFaddend Here, we compare the \DIFdelbegin \DIFdel{statistical power }\DIFdelend \DIFaddbegin \DIFadd{performance }\DIFaddend of analyses using transformed data\DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend non-parametric methods \DIFdelbegin \DIFdel{to analyses using appropriate distributional assumptions, }%DIFDELCMD < \allowbreak %%%
\DIFdel{namely Generalised Linear Models (GLM)}\DIFdelend \DIFaddbegin \DIFadd{and GLMs}\DIFaddend .

We simulated data mimicking ecotoxicological experiments of two common data types (counts and proportions \DIFaddbegin \DIFadd{from counts}\DIFaddend ). 
...
GLMs should become a standard method in ecotoxicology. \DIFaddbegin \todo{letzter absatz noch überarbeiten...}
 \DIFaddend \keywords{Generalized Linear Models \and Transformations \and Simulation \and Power \and Type 1 error}
\end{abstract}
...
\DIFaddbegin \DIFadd{Though we did not simulated overdispersed binomial data, this should be checked and accounted for.
In such situations a GLMM may offer an appealing alternative }\citep{warton_arcsine_2011}\DIFadd{.
}\DIFaddend Nevertheless, for deriving LOECs the \DIFdelbegin \DIFdel{transformation }\DIFdelend \DIFaddbegin \DIFadd{$LM$ }\DIFaddend performed better at low sample sizes (n = 3) (Figure \ref{fig:p_loec_p}). 
...
}\DIFaddend We recommend that \DIFdelbegin \DIFdel{non-normal data }\DIFdelend \DIFaddbegin \DIFadd{counts or proportions from counts }\DIFaddend should be analysed by GLMs and not by transformations or non-parametric methods.
