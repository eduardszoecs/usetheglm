...
\subsection{Simulations}
\subsubsection{Count data}
For detecting a general treatment effect $GLM_{nb}$ and $GLM_{p}$ showed inflated type 1 error rates, whereas $KW$ was conservative at low sample sizes (Figure \ref{fig:p_glob_c}).
...
\begin{figure*}
  ...
  \caption{Count data simulations (1000 simulated datasets): Type 1 error (top) and Power (bottom) for the test of a treatment effect.
...
For LOEC determination $GLM_{nb}$ and $GLM_{p}$ showed an increased Type 1 error and all other methods being slightly conservative (Figure \ref{fig:p_loec_c}).
The inferences on LOEC generally showed less power.
$LM$ showed a mean reduction of 20.7\% and $GLM_qp$ 24.3 \%.
Power to detect the LOEC was highest for $GLM_{qp}$. $LM$ and $WT$ showed less power, with $WT$ having no power to detect the LOEC at low sample sizes.


\begin{figure*}
  \centering
  \includegraphics[width = 129mm]{p_loec_c.pdf}
  \caption{Count data simulations (1000 simulated datasets): Type 1 error (top) and Power (bottom) for determination of LOEC.
