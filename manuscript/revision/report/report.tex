...
A much bigger impact had the type of multiple comparison: \citet{brock_minimum_2015} used a one-sided Williams test, whereas we used one-sided comparisons to the control (Dunnett contrasts).
In contrast to the Williams type contrasts, Dunnett contrasts do not assume a monotonic dose-response relationship and allows individual comparisons between treatment groups and the control.
However, but has under monotonicity they have less power which explain the differences \citep{jaki_statistical_2013}.
Both contrasts are available in a GLM framework.
