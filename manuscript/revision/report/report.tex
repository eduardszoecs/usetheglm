...
% % then document
% latexdiff --disable-citation-markup --math-markup=3 -t CTRADITIONAL submission/report/report.tex revision/report/report.tex > revision/diff/diff.tex
...
%% ----------------------------------------------------------------------------
\begin{abstract}
Counts and proportions are data types of encountered by ecotoxicologists.
...
Generalised Linear Models (GLM) allow to model directly distributions fitting to such data.
Here, we compare the performance of methods assuming normality of transformed data, methods assuming appropriate distributions (Poisson, negativ binomial, binomial) and non-parametric methods.

We simulated data mimicking low replicated ecotoxicological experiments of two common data types (counts and proportions from counts). 
...
For counts, we found that the quasi-Poisson model and the negative binomial model in combination with parametric boostrap had higher statistical power then data transformation.
...
Therefore, $\beta$ describes the differences between treatments also on a log scale (eqn. \ref{eqn:pois}).
...
This model assumes that the number of occurences are binomially distributed, where N = number of trials (e.g. exposed animals) and $\pi_i$ is the probability of occurrences (fish survived), giving together the expected number of occurrences.
...
%% ---- Guide GLMs
Overdispersion is common with ecological datasets \citep{warton_many_2005} and the case study illustrates the potential effects of overdispersion that is not accounted for: standard errors will be underestimated and significance overestimated (Figures \ref{fig:example}).
...
Statistical power to detect the correct LOEC was even lower (less than 30\%), which can be attributed to multiple testing.
The low power of all methods for detecting the LOEC suggests that population level NOECs reported from mesocosm experiments should be interpreted with caution and underpins the criticism of NOEC \citep{laskowski_good_1995,landis_well_2011}.
...
However, \citet{ter_braak_topics_2014} argue to use data transformations with community data because of their easy- and robustness.
...
$GLM_{qp}$ showed higher statistical power than $GLM_{npb}$ (Figure \ref{fig:p_glob_c}, top).
$GLM_{qp}$ assumes a simpler (linear) mean-variance relationship than the quadrativ relationship in $GLM_{npb}$. 
This might explain these differences between these two methods, because at small samples sizes, low abundances or few treatment groups it is difficult to determine the mean-variance relationship.
\todo{Absatz nochmal drüberlesen}
...
Though we did not simulated overdispersed binomial data, this should be checked and accounted for.
...
This can explain why $LM$ performed better for deriving LOECs at low sample sizes .

GLMs can be fitted with many statistical software packages and a lot of textbooks are available to introduce ecotoxicologists to these models (e.g. \citealt{zuur_beginners_2013} or \citealt{quinn_experimental_2009}).
