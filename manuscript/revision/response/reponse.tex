...
\response{We shortened this section. However, we sticked with a short introduction to LOEC/NOEC as inference on parameters has also been investigated by our simulations.}
...
\response{\todo{changed in code to holm -> mention in text!}}
...
\response{Such small sample sizes are common in regulatory ecotoxicology. For example \todo{finalize}}
...
\response{We agree and rerun our simulations generating 1000 datasets for all scenarios.\todo{add computation time here}}
...
\response{\todo{Convergence problems may be often encountered by ecotoxicologists...}See also response to comment 13.}
...
\todo{Take this paper also into discussion.} \todo{See also the emphasized discussion on failing to account for overdispersion.}
...
\response{We rerun our simulations and also included the Poisson GLM. The results for Poisson GLM are now also display in Figures 2+3. \todo{Moreover, we discussed failing to account for overdispersion.}}
