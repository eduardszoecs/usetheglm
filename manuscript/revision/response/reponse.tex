...
\response{We agree and included transformation bias in our discussion.}
...
For the submission we did not use Holm's method for the binomial data simulations. However, we unified this with the rerun of simulations (see comment 14).}
...
\response{We are thankful for point us to the missing reference to supplement 2. We added this to the methods section and pointed to introductory textbooks at the end of the manuscript. }
...
\response{Yes, we find it much easier to choose appropriate distributions than from infinitely possibilities of transformations. We agree, that GLMs give predictions in sound with the data. But note, that \citet{warton_many_2005} found that "\emph{a Gaussian model based on transformed abundances fitted
...
\response{We now point the reader to overdispersed binomial data, though our simulation design does not cover these cases.}
...
\response{We reordered and edited the respective results and discussion sections and emphasize more on this.}
...
The results for Poisson GLM are now also display in Figures 2+3 and we discuss the consequences of failing to account for overdisperion.}
