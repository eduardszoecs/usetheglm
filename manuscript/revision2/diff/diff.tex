...
%DIF ADD revision2/report/report.tex   Mon Apr 20 14:39:57 2015
...
%% ----------------------------------------------------------------------------
\begin{abstract}
Counts and proportions are data types \DIFdelbegin \DIFdel{often encountered by ecotoxicologists, }\DIFdelend which are rarely normally distributed\DIFaddbegin \DIFadd{, but often encountered by ecotoxicologists}\DIFaddend .
To meet the assumptions of \DIFdelbegin \DIFdel{normality and heteroscedasticity, the standard procedure has been to either transform the data or use }\DIFdelend \DIFaddbegin \DIFadd{the linear model such data are usually transformed or }\DIFaddend non-parametric methods \DIFaddbegin \DIFadd{are used }\DIFaddend if this fails.
...
Here, we compare the performance of  \DIFdelbegin \DIFdel{parametric methods assuming }\DIFdelend (1) \DIFaddbegin \DIFadd{the linear model (assuming }\DIFaddend normality of transformed data\DIFaddbegin \DIFadd{)}\DIFaddend , (2) \DIFdelbegin \DIFdel{appropriate distributions (Poisson, negativ binomial , binomial}\DIFdelend \DIFaddbegin \DIFadd{GLMs (assuming a Poisson, negative binomial or binomially distibuted response}\DIFaddend ) and (3) non-parametric methods.
...
For \DIFdelbegin \DIFdel{counts}\DIFdelend \DIFaddbegin \DIFadd{count data}\DIFaddend , we found that the quasi-Poisson model \DIFdelbegin \DIFdel{and the }\DIFdelend \DIFaddbegin \DIFadd{yielded the highest power. The }\DIFaddend negative binomial model \DIFdelbegin \DIFdel{in combination with the parametric boostrap had higher statistical power than data transformation}\DIFdelend \DIFaddbegin \DIFadd{resulted in increased Type I errors, which could be fixed using the parametric bootstrap}\DIFaddend . 
...
The compared non-parametric methods had generally \DIFdelbegin \DIFdel{lower }\DIFdelend \DIFaddbegin \DIFadd{lowered }\DIFaddend power.
...
\DIFaddbegin \DIFadd{When applied to untransformed data, the linear model would result in identical predicted treatment means as GLMs. 
