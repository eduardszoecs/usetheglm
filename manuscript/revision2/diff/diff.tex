...
%DIF ADD revision2/report/report.tex   Fri Apr 17 17:42:48 2015
...
We compared the performance of different methods in terms of statistical power and \DIFdelbegin \DIFdel{type 1 }\DIFdelend \DIFaddbegin \DIFadd{Type I }\DIFaddend error for detecting a general treatment effect and determining the lowest observed effect concentration (LOEC).
...
We recommend that counts and proportions from counts should be analysed by \DIFdelbegin \DIFdel{making appropriate distributional assumptions and GLMs should become a standard method }\DIFdelend \DIFaddbegin \DIFadd{GLMs and these methods should become standard }\DIFaddend in ecotoxicology.
...
Recent studies concluded that data transformations should be avoided and GLMs be used as they have better statistical properties (\DIFdelbegin %DIFDELCMD < \citealt{ohara_not_2010} %%%
...
, where $y_i$ is the measured and \DIFdelbegin \DIFdel{$y_i^T$ }\DIFdelend \DIFaddbegin \DIFadd{$y_{new~i}$ }\DIFaddend the transformed abundance of the $i$th observation. 
...
\begin{align}
  y\DIFdelbegin \DIFdel{_i^T }\DIFdelend \DIFaddbegin \DIFadd{_{new~i} }\DIFaddend &\sim N(\mu_i, \sigma^2) \nonumber \\
  E(y\DIFdelbegin \DIFdel{_i^T}\DIFdelend \DIFaddbegin \DIFadd{_{new~i}}\DIFaddend ) = \mu_i ~&\text{and}~ var(y\DIFdelbegin \DIFdel{_i^T}\DIFdelend \DIFaddbegin \DIFadd{_{new~i}}\DIFaddend ) = \sigma^2 \label{eqn:normal} \\
  \mu_i &= \beta \DIFdelbegin \DIFdel{Treatment}\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend _i  \nonumber
\end{align}
...
We allow this mean to vary between treatments \DIFaddbegin \DIFadd{($x_i$ codes the treatments) }\DIFaddend and $\beta$ are the coefficients related to these changes in transformed abundances between treatments (eqn. \ref{eqn:normal}).
...
\begin{align}
  y\DIFdelbegin \DIFdel{_i^T }\DIFdelend \DIFaddbegin \DIFadd{_{new~i} }\DIFaddend = 
  \begin{cases}  
...
\begin{align}
  ...
  logit~(\pi_i) &= \beta \DIFdelbegin \DIFdel{Treatment}\DIFdelend \DIFaddbegin \DIFadd{x}\DIFaddend _i \nonumber
  \end{align}
...
For Type \DIFdelbegin \DIFdel{1 }\DIFdelend \DIFaddbegin \DIFadd{I }\DIFaddend error estimation, $\pi_C$ was \DIFdelbegin \DIFdel{held constant between groups}\DIFdelend \DIFaddbegin \DIFadd{equal between treatments}\DIFaddend .
...
%% --------------------------------
\subsection{Case study}
The data set showed \DIFdelbegin \DIFdel{considerable }\DIFdelend \DIFaddbegin \DIFadd{considerably }\DIFaddend higher variance then expected by the Poisson model ($\Theta = 22.41$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend eqn. \ref{eqn:quasi})\DIFdelbegin \DIFdel{. 
...
Nevertheless, such a Williams-type MCT is not a panacea }\citep{hothorn_statistical_2014} \DIFadd{and our simulated semi-concave dose-response relationship is a situation where it fails and underestimates the LOEC }\citep{kuiper_identification_2014}\DIFaddend . 
...
\DIFaddbegin \DIFadd{As our simulations showed the LR test for $GLM_{nb}$ is invalid because of increased Type I errors. This explains why it had the lowest p-value in the case study.
}
...
However, bootstrapping is computationally very intensive and we found no gains in power compared to $GLM_{qp}$ (Figure \ref{fig:p_glob_c}). Because of increased higher power, appropriate Type I errors and reduced bias }\citep{ohara_not_2010} \DIFadd{we conclude that count data in one factorial experiments should be analysed using the quasi-Poisson model}\DIFaddend .
