...
%DIF ADD revision2/report/report.tex   Mon Apr 20 20:30:30 2015
...
%% ----------------------------------------------------------------------------
\begin{abstract}
\DIFdelbegin \DIFdel{Counts and proportions are data types often encountered by ecotoxicologists, which }\DIFdelend \DIFaddbegin \DIFadd{Ecotoxicologists often encounter count and proportion data that }\DIFaddend are rarely normally distributed.
To meet the assumptions of \DIFdelbegin \DIFdel{normality and heteroscedasticity, the standard procedure has been to either transform the data or use }\DIFdelend \DIFaddbegin \DIFadd{the linear model such data are usually transformed or }\DIFaddend non-parametric methods \DIFdelbegin \DIFdel{if this fails}\DIFdelend \DIFaddbegin \DIFadd{are used if the transformed data still violate the assumptions}\DIFaddend .
Generalised Linear Models (GLM) allow \DIFdelbegin \DIFdel{directly model distributions fitting such data}\DIFdelend \DIFaddbegin \DIFadd{to directly model such data, without the need for transformation}\DIFaddend .
Here, we compare the performance of  \DIFdelbegin \DIFdel{parametric methods assuming }\DIFdelend \DIFaddbegin \DIFadd{two parametric methods i.e.  }\DIFaddend (1) \DIFaddbegin \DIFadd{the linear model (assuming }\DIFaddend normality of transformed data\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{), and }\DIFaddend (2) \DIFdelbegin \DIFdel{appropriate distributions (Poisson, negativ binomial , binomial}\DIFdelend \DIFaddbegin \DIFadd{GLMs (assuming a Poisson, negative binomial or binomially distibuted response}\DIFaddend ) and (3) non-parametric methods.
...
For \DIFdelbegin \DIFdel{counts}\DIFdelend \DIFaddbegin \DIFadd{count data}\DIFaddend , we found that the quasi-Poisson model \DIFdelbegin \DIFdel{and the negative binomial model in combination with the parametric boostrap had higher statistical power than data transformation}\DIFdelend \DIFaddbegin \DIFadd{yielded the highest power. The negative binomial GLM resulted in increased Type I errors, which could be fixed using the parametric bootstrap}\DIFaddend . 
For proportions\DIFaddbegin \DIFadd{, binomial }\DIFaddend GLMs performed better \DIFaddbegin \DIFadd{than the linear model}\DIFaddend , except to determine LOEC at \DIFdelbegin \DIFdel{extremly }\DIFdelend \DIFaddbegin \DIFadd{extremely }\DIFaddend low sample sizes.
...
For detecting a treatment effect,\DIFdelbegin \DIFdel{$GLM_{npb}$ and }\DIFdelend $GLM_{qp}$ \DIFdelbegin \DIFdel{exhibited higher powerthan }\DIFdelend \DIFaddbegin \DIFadd{had the highest power, followed by $GLM_{npb}$, }\DIFaddend $LM$ and $KW$, the latter having least power (Figure \ref{fig:p_glob_c}).
...
\begin{figure*}
  ...
  \DIFdelbeginFL \DIFdelFL{Only type 1 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Type I }\DIFaddendFL errors \DIFdelbeginFL \DIFdelFL{\textless 25\% }\DIFdelendFL are displayed \DIFaddbeginFL \DIFaddFL{on a logarithmic scale}\DIFaddendFL .
  \DIFdelbeginFL \DIFdelFL{$GLM_p$ showed type 1 errors \textgreater 20\% in all simulation scenarios.
  }\DIFdelendFL Power levels for models with inflated \DIFdelbeginFL \DIFdelFL{type }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Type }\DIFaddendFL I \DIFdelbeginFL \DIFdelFL{error }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{errors ($GLM_P$ and $GLM_{qp}$) }\DIFaddendFL are shown for completeness.
...
\begin{figure*}
  ...
  \DIFdelbeginFL \DIFdelFL{For clarity only type 1 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Type I }\DIFaddendFL errors \DIFdelbeginFL \DIFdelFL{\textless 25\% }\DIFdelendFL are displayed \DIFaddbeginFL \DIFaddFL{one a logarithmic scale}\DIFaddendFL .
...
\DIFdelbegin \DIFdel{Nevertheless, }\DIFdelend \citet{ohara_not_2010} showed that \DIFdelbegin \DIFdel{$LM$ using a log transformation }\DIFdelend \DIFaddbegin \DIFadd{the linear model on log transformed data }\DIFaddend gave unreliable and biased \DIFdelbegin \DIFdel{parameter }\DIFdelend estimates, whereas GLMs performed well with little bias.
Bias occurs also when back-transforming \DIFaddbegin \DIFadd{fitted }\DIFaddend means to the original scale, which explains the lower predicted means by $LM$ in Figure \ref{fig:example} \citep{rothery_cautionary_1988} and should be corrected for \citep{newman_regression_1993}.
\DIFaddbegin \DIFadd{When applied to non-transformed data, the linear model would predict identical treatment means as GLMs, because for a categorical predictor the predicted means of the LM and GLM are identical. 
When applied to non-transformed data, the linear model would result in identical predicted treatment means as GLMs. 
However, predictions would differ with a continuous predictor and GLMs particularly advantageous in this case.
}\DIFaddend 
...
However, in factorial designs the mean-variance relationship can be easily checked by plotting mean versus variance of the treatment groups \DIFaddbegin \DIFadd{or by inspecting residual versus fitted values plots }\DIFaddend (see Supplement 2).
...
Given the higher power, appropriate Type I errors, stable convergence and reduced bias }\citep{ohara_not_2010} \DIFadd{we suggest that count data in one factorial experiments should be analysed using the quasi-Poisson model}\DIFaddend .
