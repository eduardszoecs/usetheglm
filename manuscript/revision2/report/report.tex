...
%% ----------------------------------------------------------------------------
\begin{abstract}
Counts and proportions are data types which are rarely normally distributed, but often encountered by ecotoxicologists.
To meet the assumptions of the linear model such data are usually transformed or non-parametric methods are used if this fails.
Generalised Linear Models (GLM) allow to directly model such data, without the need for transformation.
Here, we compare the performance of  (1) the linear model (assuming normality of transformed data), (2) GLMs (assuming a Poisson, negative binomial or binomially distibuted response) and (3) non-parametric methods.
...
For count data, we found that the quasi-Poisson model yielded the highest power. The negative binomial model resulted in increased Type I errors, which could be fixed using the parametric bootstrap. 
...
The compared non-parametric methods had generally lowered power.
...
When applied to untransformed data, the linear model would result in identical predicted treatment means as GLMs. 
