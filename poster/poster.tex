...
\begin{column}{.48\linewidth}
    ...
    \begin{exampleblock}{Most eco(toxico)logical data is not normally distributed}
        ...
        \begin{itemize}
            \item Usually analysed by \\[0.5ex]
                \begin{itemize}[label=$-$]
                    \item transformation (e.g. $log(Ax + C)$, $x^{0.5}, arcsine~x^{0.25}$) for linear model \cite{newman_quantitative_2012}
                    \item non-parametric methods \cite{wang_making_2011}
                \end{itemize}
                \vskip1ex
            \item \emph{Generalized Linear Models} (GLM) an alternative to directly model such data
    ...
    \begin{block}{Results: Type I errors}
        \begin{figure}
             ...
             \caption{Type 1 errors for testing global treatment effect. }
         \end{figure}
         \vskip2ex
        ...
        \noindent\makebox[\textwidth][c]{%
         ...
         \begin{enumerate}[label=\numberingI{\arabic*}, itemindent=0cm, itemsep = 1.5ex]
            ...
            \item Kruskal-Wallis test  is slightly conservative
         \end{enumerate}
        \end{minipage}
         }
    \end{block}
